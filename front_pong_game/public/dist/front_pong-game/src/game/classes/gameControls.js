...
export class GameControls {
    constructor(paddle1, paddle2, ball, playerNumber, socket, babylonManager) {
        this.keysPressed = {};
        this.lastFrameTime = 0;
        this.inputSequence = 1;
        this.inputHistory = [];
        this.remotePaddleBuffer = [];
        this.paddleInterpolationDelay = 100;
        this.ballPositionBuffer = [];
        this.ballInterpolationDelay = 100;
        this.networkMeasurements = [];
        this.avgPacketInterval = 1000 / GameConfig.BROADCAST_RATE;
    ...
    sendPaddleMovements(deltaTime = 16.67) {
    ...
    updateLocalPaddlePosition(deltaTime) {
        this.localPaddle.move(deltaTime);
    }
    updateRemotePaddlePosition() {
    ...
    updateServerPaddlePosition(serverPaddle1, serverPaddle2) {
    ...
    storeRemotePosition(serverRemotePaddle) {
        const now = performance.now();
    ...
    reconcilePaddlePosition(serverLocalPaddle) {
    ...
    storeBallPosition(serverBall) {
        ...
        if (this.ballPositionBuffer.length > 0) {
            ...
            if (this.networkMeasurements.length >= 5) {
                ...
                this.ballInterpolationDelay = Math.min(Math.max(this.avgPacketInterval * 2, 50), // Min 50ms
                150 // Max 150ms
                );
            }
        }
    ...
    cleanupBuffer() {
        ...
        const now = performance.now();
        this.ballPositionBuffer = this.ballPositionBuffer.filter(point => now - point.time < maxAgeMs);
    }
