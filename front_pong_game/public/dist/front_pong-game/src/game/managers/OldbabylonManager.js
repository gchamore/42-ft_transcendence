...
export class BabylonManager {
    ...
    createMeshes() {
        ...
        this.paddleMesh1.position = new BABYLON.Vector3((this.paddle1.x + this.paddle1.width / 2 - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, // Center of paddle
        -0.25, (GameConfig.CANVAS_HEIGHT / 2 - this.paddle1.y) * this.scaleY);
        ...
        this.paddleMesh2.position = new BABYLON.Vector3((this.paddle2.x + this.paddle2.width / 2 - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, // Center of paddle
        -0.25, (GameConfig.CANVAS_HEIGHT / 2 - this.paddle2.y) * this.scaleY);
    ...
    createPowerupMesh(powerup) {
        if (!this.scene)
            return;
        const position = new BABYLON.Vector3((powerup.x - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, -0.2, (GameConfig.CANVAS_HEIGHT / 2 - powerup.y / 2) * this.scaleY);
        let powerupPath = "";
        ...
        BABYLON.SceneLoader.ImportMesh("", "/assets/models/", powerupPath, this.scene, (meshes) => {
            if (meshes.length > 0 && this.scene) {
                ...
                const collisionSphere = BABYLON.MeshBuilder.CreateSphere(`powerup-collision-${powerup.id}`, {
    ...
    getScaleFactorForModel(powerupType) {
        var _a;
        const scales = GameConfig.POWERUP_VISUAL_SCALES;
        return (_a = scales[powerupType]) !== null && _a !== void 0 ? _a : 0.1;
    }
    ...
    updatePaddleMeshDimension(paddleMesh, paddleState) {
        var _a;
        if (!this.scene)
            return;
        ...
        paddleMesh.position = new BABYLON.Vector3((paddleState.x + paddleState.width / 2 - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, -0.25, (GameConfig.CANVAS_HEIGHT / 2 - paddleState.y) * this.scaleY);
        this.updateCollisionBoxDimension(paddleMesh, paddleState);
    }
    updateCollisionBoxDimension(paddleMesh, paddleState) {
        if (!this.paddleCollisionBox1 || !this.paddleCollisionBox2 || !this.paddleMesh1 || !this.paddleMesh2)
            return;
        ...
        collisionBox.position = new BABYLON.Vector3((paddleState.x + paddleState.width / 2 - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, -0.25, (GameConfig.CANVAS_HEIGHT / 2 - paddleState.y) * this.scaleY);
    }
    updateBallMeshDimension(ballState) {
        if (!this.scene || !this.ballMesh)
            return;
    ...
    updatePaddleMeshPositions() {
        if (!this.paddleMesh1 || !this.paddleMesh2)
            return;
        this.paddleMesh1.position = new BABYLON.Vector3((this.paddle1.x + this.paddle1.width / 2 - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, -0.25, (GameConfig.CANVAS_HEIGHT / 2 - this.paddle1.y) * this.scaleY);
        this.paddleMesh2.position = new BABYLON.Vector3((this.paddle2.x + this.paddle2.width / 2 - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, -0.25, (GameConfig.CANVAS_HEIGHT / 2 - this.paddle2.y) * this.scaleY);
    }
    updateBallMeshPosition() {
        if (!this.ballMesh)
            return;
        const targetPosition = new BABYLON.Vector3((this.ball.x - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, -0.2, (GameConfig.CANVAS_HEIGHT / 2 - this.ball.y) * this.scaleY);
        this.ballMesh.position = targetPosition;
    }
    handleBounce(position) {
        if (!this.scene)
            return;
        const bouncePosition = new BABYLON.Vector3((position.x - (GameConfig.CANVAS_WIDTH / 2)) * (20 / GameConfig.CANVAS_WIDTH), -0.2, ((GameConfig.CANVAS_HEIGHT / 2) - position.y) * (15 / GameConfig.CANVAS_HEIGHT));
        this.addBounceEffect(bouncePosition);
    }
    addBounceEffect(position) {
        if (!this.scene)
            return;
        const particleSystem = new BABYLON.ParticleSystem("particles", 300, this.scene);
    ...
    handlePaddleHit(playerNumber, ballPosition) {
        ...
        const targetPaddleMesh = playerNumber === 1 ? this.paddleMesh1 : this.paddleMesh2;
        if (targetPaddleMesh) {
            const originalMaterial = targetPaddleMesh.material;
    ...
    handleScoreEffect(scorer, position) {
        ...
        const scoreMesh = BABYLON.MeshBuilder.CreateSphere("scoreEffect", { diameter: 2 }, this.scene);
        scoreMesh.position = new BABYLON.Vector3((position.x - (GameConfig.CANVAS_WIDTH / 2)) * (20 / GameConfig.CANVAS_WIDTH), 0, ((GameConfig.CANVAS_HEIGHT / 2) - position.y) * (15 / GameConfig.CANVAS_HEIGHT));
    ...
    createCollisionBoxes() {
        ...
        this.paddleCollisionBox1 = BABYLON.MeshBuilder.CreateBox("paddle1Collision", {
        ...
        this.paddleCollisionBox2 = BABYLON.MeshBuilder.CreateBox("paddle2Collision", {
    ...
    updateCollisionBoxPositions() {
        if (!this.paddleCollisionBox1 || !this.paddleCollisionBox2)
            return;
        // Position collision box for paddle 1
        this.paddleCollisionBox1.position = new BABYLON.Vector3((this.paddle1.x + this.paddle1.width / 2 - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, -0.25, (GameConfig.CANVAS_HEIGHT / 2 - this.paddle1.y) * this.scaleY);
        // Position collision box for paddle 2
        this.paddleCollisionBox2.position = new BABYLON.Vector3((this.paddle2.x + this.paddle2.width / 2 - GameConfig.CANVAS_WIDTH / 2) * this.scaleX, -0.25, (GameConfig.CANVAS_HEIGHT / 2 - this.paddle2.y) * this.scaleY);
    }
    dispose() {
        this.hideLoadingScreen();
