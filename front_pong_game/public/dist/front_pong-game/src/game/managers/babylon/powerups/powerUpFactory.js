...
export class PowerUpFactory {
    constructor(scene, coordinateConverter, effectsManager) {
        this.scene = scene;
    ...
    createPowerupMesh(powerup) {
        ...
        const position = this.coordinateConverter.toScenePosition(powerup.x, powerup.y, -0.2);
        let powerupPath = "";
    ...
    createCollisionSphere(powerupId, emissiveColor, position) {
        if (!this.scene)
            return;
        const collisionSphere = BABYLON.MeshBuilder.CreateSphere(`powerup-collision-${powerupId}`, {
