...
export class SettingsService {
	static loadSettings() {
		const ballSpeed =
			localStorage.getItem("ballSpeed") ||
			GameConfig.DEFAULT_BALL_SPEED.toString();
		const paddleSpeed =
			localStorage.getItem("paddleSpeed") ||
			GameConfig.DEFAULT_PADDLE_SPEED.toString();
		const paddleLength =
			localStorage.getItem("paddleLength") ||
			GameConfig.DEFAULT_PADDLE_LENGTH.toString();
		const mapType =
			localStorage.getItem("mapType") || GameConfig.DEFAULT_MAP;
		const powerUpsEnabled =
			localStorage.getItem("powerUpsEnabled") === "true" ||
	...
	static saveSettings(settings: any) {
		...
		localStorage.setItem(
			"powerUpsEnabled",
			settings.powerUpsEnabled.toString()
		);
	}
}
